...
class wpcmsb_cmsblock {
	...
	public static function register_post_type() {
		register_post_type( self::post_type, array(
			'labels' => array(
				'name' => __( 'CMS Block', 'cms-block' ),
				'singular_name' => __( 'CMS Block', 'cms-block' ) ),
			'rewrite' => false,
			'query_var' => false ) );
	}
	...
	private static function get_unit_tag( $id = 0 ) {
		...
		if ( in_the_loop() ) {
			$unit_tag = sprintf( 'wpcmsb-b%1$d-p%2$d-o%3$d',
				absint( $id ), get_the_ID(), $global_count );
		} else {
			$unit_tag = sprintf( 'wpcmsb-b%1$d-o%2$d',
	...
	public function form_html( $atts = array() ) {
		...
		$html = sprintf( '<div %s>', wpcmsb_format_atts( array(

			'class' => 'wpcmsb',
			'id' => $this->unit_tag,
			'dir' => wpcmsb_is_rtl( $this->locale ) ? 'rtl' : 'ltr' ) ) ) . "\n";
	...
	private function form_hidden_fields() {
		$hidden_fields = array(
			'_wpcmsb' => $this->id,
			'_wpcmsb_version' => wpcmsb_VERSION,
			'_wpcmsb_locale' => $this->locale,
			'_wpcmsb_unit_tag' => $this->unit_tag );

		if ( wpcmsb_VERIFY_NONCE )
			$hidden_fields['_wpnonce'] = wpcmsb_create_nonce( $this->id );
	...
	public function form_response_output() {
		$class = 'wpcmsb-response-output';
		$role = '';
		$content = '';

                $class .= ' wpcmsb-display-none';
		...
		$output = apply_filters( 'wpcmsb_form_response_output',
	...
	public function shortcode( $args = '' ) {
		...
		if ( $args['use_old_format'] ) {
			...
			if ( $old_unit_id ) {
				$shortcode = sprintf( '[cms-block %1$d "%2$s"]', $old_unit_id, $title );
			} else {
				$shortcode = '';
			}
		} else {
			$shortcode = sprintf( '[cms-block id="%1$d" title="%2$s"]',
				$this->id, $title );
		}
