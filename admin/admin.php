...
function wpcmsb_admin_menu() {
	add_object_page( __( 'CMS Block', 'cms-block' ),
		__( 'Blocks', 'cms-block' ),
		'wpcmsb_read_cms_blocks', 'wpcmsb',
		'wpcmsb_admin_management_page', 'dashicons-screenoptions' );

	$edit = add_submenu_page( 'wpcmsb',
		__( 'Edit CMS Block', 'cms-block' ),
	...
	$addnew = add_submenu_page( 'wpcmsb',
		__( 'Add New CMS Block', 'cms-block' ),
		__( 'Add New Block', 'cms-block' ),
		'wpcmsb_edit_cms_blocks', 'wpcmsb-new',
	...
	if ( $integration->service_exists() ) {
		$integration = add_submenu_page( 'wpcmsb',
...
function wpcmsb_load_cms_block_admin() {
	...
	if ( 'save' == $action ) {
		...
		$redirect_to = add_query_arg( $query, menu_page_url( 'wpcmsb', false ) );
	...
	if ( 'wpcmsb-new' == $plugin_page && isset( $_GET['locale'] ) ) {
	...
	} else if ( 'wpcmsb-new' == $plugin_page ) {
...
function wpcmsb_admin_enqueue_scripts( $hook_suffix ) {
	if ( false === strpos( $hook_suffix, 'wpcmsb' ) ) {
		return;
	}
...
<h2><?php
	echo esc_html( __( 'CMS Block', 'cms-block' ) );
...
<form method="get" action="">
	...
	<?php $list_table->search_box( __( 'Search CMS Block', 'cms-block' ), 'wpcmsb-contact' ); ?>
...
<h2><?php echo esc_html( __( 'Add New CMS Block', 'cms-block' ) ); ?></h2>

<?php do_action( 'wpcmsb_admin_notices' ); ?>

<p><a href="<?php echo esc_url( add_query_arg( array( 'locale' => $default_locale ), menu_page_url( 'wpcmsb-new', false ) ) ); ?>" class="button button-primary" /><?php echo esc_html( __( 'Add New', 'cms-block' ) ); ?></a></p>

<?php unset( $available_locales[$default_locale] ); ?>
<form action="" method="get">
<input type="hidden" name="page" value="wpcmsb-new" />
</form>
</div>
<?php
}
...
function wpcmsb_admin_updated_message() {
	...
	if ( 'created' == $_REQUEST['message'] )
		$updated_message = esc_html( __( 'CMS Block created.', 'cms-block' ) );
	elseif ( 'saved' == $_REQUEST['message'] )
		$updated_message = esc_html( __( 'CMS Block saved.', 'cms-block' ) );
	elseif ( 'deleted' == $_REQUEST['message'] )
		$updated_message = esc_html( __( 'CMS Block deleted.', 'cms-block' ) );
...
function wpcmsb_plugin_action_links( $links, $file ) {
	...
	$settings_link = '<a href="' . menu_page_url( 'wpcmsb', false ) . '">'
...
function wpcmsb_old_wp_version_error() {
	...
	if ( 'wpcmsb' != substr( $plugin_page, 0, 5 ) ) {
		return;
	}
...
?>
<div class="error">
<p><?php echo sprintf( __( '<strong>CMS Block %1$s requires WordPress %2$s or higher.</strong> Please <a href="%3$s">update WordPress</a> first.', 'cms-block' ), wpcmsb_VERSION, wpcmsb_REQUIRED_WP_VERSION, admin_url( 'update-core.php' ) ); ?></p>
</div>
<?php
}
...
function wpcmsb_welcome_panel() {
	...
	if ( 'wpcmsb' != $plugin_page || ! empty( $_GET['post'] ) ) {
		return;
	}
...
<div id="welcome-panel" class="<?php echo esc_attr( $classes ); ?>">
	...
	<a class="welcome-panel-close" href="<?php echo esc_url( menu_page_url( 'wpcmsb', false ) ); ?>"><?php echo esc_html( __( 'Dismiss', 'cms-block' ) ); ?></a>

	<div class="welcome-panel-content">
		<div class="welcome-panel-column-container">
			<div class="welcome-panel-column">
				<h4><?php echo esc_html( __( 'CMS Block Needs Your Support', 'cms-block' ) ); ?></h4>
				<p class="message"><?php echo esc_html( __( "It is hard to continue development and support for this plugin without contributions from users like you. If you enjoy using CMS Block and find it useful, please consider making a donation.", 'cms-block' ) ); ?></p>
...
function wpcmsb_not_allowed_to_edit() {
	...
	$message = __( "You are not allowed to edit this CMS Block.",
		'cms-block' );
